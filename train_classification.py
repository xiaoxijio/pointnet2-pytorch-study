...
def parse_args():
    ...
    parser.add_argument('--use_normals', action='store_true', default=True, help='用不用法向量信息')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader),
            ...
            points = provider.random_point_dropout(points)  # 随机丢掉一些点
            points[:, :, 0:3] = provider.random_scale_point_cloud(points[:, :, 0:3])  # 抖动
