...
def square_distance(src, dst):
    ...
    """  # dist = (src - dst)^2 =  -2*sec*dst + src^2 + dst^2
    B, N, _ = src.shape  # [B, N, C]
    _, M, _ = dst.shape  # [B, M, C]
    dist = -2 * torch.matmul(src, dst.permute(0, 2, 1))  # [B, M, C] -> [B, C, M] * [B, N, C] -> [B, N, M]
    dist += torch.sum(src ** 2, -1).view(B, N, 1)  # 将结果扩展为 [B, N, 1]，便于与最终结果形状 [B, N, M] 对齐
